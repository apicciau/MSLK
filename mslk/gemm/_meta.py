...
# separate Buck targets (cutlass_bf16bf16bf16_grouped_grad/wgrad). If the
# native library fails to load (e.g., in restricted build environments), the
# C++ schemas won't be registered. Guard with hasattr so this module still
# imports cleanly.
if hasattr(torch.ops.mslk, "bf16bf16bf16_grouped_grad"):
...
if hasattr(torch.ops.mslk, "bf16bf16bf16_grouped_wgrad"):
